...
public:
	...
	void AddTexture(int indice, char* url);
	void SetColor(int indiceObject, int indiceColor);
	void SetTexture(int indiceObject, int indiceTexture);
...
private:
	...
	int m_textureObject[NUMBER_OBJECTS] = { -1 };
	...
	GLuint m_textures[NUMBER_COLORS];
	int m_haveTexture[NUMBER_COLORS];
};
