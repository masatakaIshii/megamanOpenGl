...
void Megaman::Draw(){
    ...
    glTranslatef(moveX, jumpY, moveZ);
    if(etapeDeplacement == 1)
    {
        glRotatef(90,0,1,0);
    }
    else if(etapeDeplacement == 3)
    {
        glRotatef(-90,0,1,0);
    }
    else if(etapeDeplacement == 4)
    {
        glRotatef(180,0,1,0);
    }
	...
	if (bullet && shoot && canon) {
		...
		if (now >= seconds + TIME_SHOOT)
		{
            ...
            translateBulletX[keepPuissance.size()] = 2.8 +  moveX;
            translateBulletY[keepPuissance.size()] = jumpY + 1.5;
            translateBulletZ[keepPuissance.size()] = 2.8 + moveZ;
            startBulletX[keepPuissance.size()] = translateBulletX[keepPuissance.size()];
            etapeBullet[keepPuissance.size()] = etapeDeplacement;
	...
	if (shootBullet) {
		...
		for (list<float>::iterator i = keepPuissance.begin(); i != keepPuissance.end(); i++)
		{
			...
			glPushMatrix();
			glTranslatef(translateBulletX[count], translateBulletY[count], translateBulletZ[count]);
...
void Megaman::SetColors(Object *megaman){
    ...
    //megaman->AddTexture(0,"img/bleu.jpg");
    //megaman->SetTexture(CORPS,0);
	...
	if (walk)
	{
		...
		if (rightLeg) {
			if (walkEtape == 1) {
				...
				if (angleForelegX < 0) {
                    if(etapeDeplacement == 0 || etapeDeplacement == 2)
                    {
                        moveZ += VITESSE_WALK;
                    }
...
void Megaman::ShootBullet(){
    if (shootBullet)
	{
		for (int i = 0; i < keepPuissance.size(); i++)
		{
		    if(etapeBullet[i] == 0)
		    {
                if (translateBulletX[i] < startBulletX[i] + 100) {
                    translateBulletX[i] += 0.4;
                    translateBulletZ[i] += 0.4;
                }
                else {
                    keepPuissance.pop_front();
                }
		    }
		    else if(etapeBullet[i] == 1)
            {
                if (translateBulletX[i] < startBulletX[i] + 100) {
                    translateBulletX[i] += 0.4;
                    translateBulletZ[i] -= 0.4;
...
void Megaman::walkInMap()
{
    if(etapeDeplacement == 0)
    {
        if(moveZ > 70)
        {
            etapeDeplacement = 1;
